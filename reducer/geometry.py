...
class ReducerGeometry:
    ...
    def min_inner_radius(self) -> float:
        return (
                ceil(1.03 * self.ball_diameter)
                / np.sin(np.pi / self.number_of_waves)
        )
    ...
    def track_outer_radius(self) -> float:
        if self.requested_track_outer_radius is not None:
            inner_radius = self._get_track_inner_radius_from_outer(
                self.requested_track_outer_radius
            )
    ...
    def xy(self) -> tuple[np.ndarray]:
        ...
        S = np.sqrt(
            ...
            - np.power(
                self.eccentricity * np.sin(self.number_of_waves * theta),
                2
            )
        )
        ...
        Xi = np.arctan2(
            ...
            * np.sin(self.number_of_waves * theta),
            S,
        )
    ...
    def balls_centers(self) -> zip:
        ...
        S_sh = np.sqrt(
            (self.balls_radius + self.eccentric_radius) ** 2
            - np.power(
                self.eccentricity * np.sin(self.number_of_waves * sh_angle),
                2
            )
        )
        l_Sh = (
                self.eccentricity * np.cos(self.number_of_waves * sh_angle)
                + S_sh
        )
