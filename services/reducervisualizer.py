...
class ReducerVisualizer:
    ...
    def _wave(self, ax):
        ...
        if (
            self.reducer_outer_diameter is None
        ):  # or self.reducer_outer_diameter < self.track_outer_radius * 2 + 5:
            reduser_outer_circle = plt.Circle(
                (0, 0),
                (self.track_outer_radius + 2),
                color="#7410ff",
                fill=False,
                linewidth=2.0,
                label="body",
    ...
    def _separator(self, ax):
        outer_separator_circle = plt.Circle(
            (0, 0),
            self.radius_of_separator_outer,
            fill=False,
            linewidth=1.0,
            label="separator",
        )
        inner_separator_circle = plt.Circle(
            (0, 0), self.radius_of_separator_inner, fill=False, linewidth=1.0
        )
